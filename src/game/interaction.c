...
u32 determine_interaction(struct MarioState *m, struct Object *o) {
    ...
    if (gSonicState.task.flag & (SF_BALL_AURA | SF_JD_AURA)) {
...
void bounce_off_object(struct MarioState *m, struct Object *o, f32 velY) {
    Vector bounce;
    if (!(gSonicState.task.flag & SF_GROUNDED))
    {
        gSonicState.task.mode = SM_AIRBORNE;
        gSonicState.spd.x = 0.0f;
        gSonicState.spd.y = 120.0f / 5.0f / gSonicState.unitScale;
        gSonicState.spd.z = 0.0f;
        gSonicState.jumpTimer = 0;
        Sonic_ConvertVector_G2P(&gSonicState, &gSonicState.spd);
...
void mario_process_interactions(struct MarioState *m) {
    ...
    if (m->invincTimer > 0 && !sDelayInvincTimer && (gSonicState.task.mode != SM_HURT && (gSonicState.task.flag & SF_DEAD) == 0)) {
        m->invincTimer -= 1;
    }
