...
void debug_print_speed_action_normal(struct MarioState *m) {
    UNUSED f32 steepness;
    UNUSED f32 floor_nY;

	if (m->controller->buttonDown & Z_TRIG) {
		print_text_fmt_int(16, 48+112, "AIR %d", gSonicState.airTime);
        print_text_fmt_int(16, 48+96, "AN1 %x", gSonicState.task.ang.x * 0x1000);
		print_text_fmt_int(16, 48+80, "AN2 %x", gSonicState.task.ang.y * 0x1000);
		print_text_fmt_int(16, 48+64, "AN3 %x", gSonicState.task.ang.z * 0x1000);
		print_text_fmt_int(16, 48+48, "AN4 %x", gSonicState.task.ang.w * 0x1000);
		print_text_fmt_int(16, 48+32, "GP1 %x", gSonicState.gspd.x * 0x1000);
		print_text_fmt_int(16, 48+16, "GP2 %x", gSonicState.gspd.y * 0x1000);
		print_text_fmt_int(16, 48,    "GP3 %x", gSonicState.gspd.z * 0x1000);
		
		print_text_fmt_int(210, 48+128, "HNT %x", gSonicState.jumpTimer);
		print_text_fmt_int(210, 48+112, "DAT %x", gSonicState.jdTimer);
		print_text_fmt_int(210, 48+96, "CHG %x", gSonicState.spindashSpeed * 0x1000);
		print_text_fmt_int(210, 48+80, "LP1 %x", gSonicState.spd.x * 0x1000);
		print_text_fmt_int(210, 48+64, "LP2 %x", gSonicState.spd.y * 0x1000);
		print_text_fmt_int(210, 48+48, "LP3 %x", gSonicState.spd.z * 0x1000);
		print_text_fmt_int(210, 48+32, "ANI %x", gSonicState.anim);
		print_text_fmt_int(290, 48+32, "M%x", gSonicState.animMode);
		print_text_fmt_int(210, 48+16, "MDE %x", gSonicState.task.mode);
		print_text_fmt_int(210, 48,    "FLG %x", gSonicState.task.flag);
	}
}
...
void update_mario_health(struct MarioState *m) {
    ...
    if (m->hurtCounter > 0)
    {
        //If we had no coins, die
        if (m->numCoins == 0)
            gSonicState.task.flag |= SF_DEAD;
        ...
        gSonicState.spd.x = -1.5f;
        gSonicState.spd.y = 1.5f;
        gSonicState.spd.z = 0.0f;
        gSonicState.task.mode = SM_HURT;
        gSonicState.task.flag &= ~SF_GROUNDED;
    }
...
void mario_update_hitbox_and_cap_model(struct MarioState *m) {
    ...
    if (m->invincTimer >= 3 && (gSonicState.task.mode != SM_HURT && (gSonicState.task.flag & SF_DEAD) == 0)) {
    ...
    */
	...
	switch (gMarioState->action)
	{
		case ACT_DISAPPEARED:
			if (gMarioState->actionArg)
			{
				gMarioState->actionArg--;
				if ((gMarioState->actionArg & 0xFFFF) == 0)
					level_trigger_warp(gMarioState, gMarioState->actionArg >> 16);
			}
			return 0;
	}
	
	//Apply Mario state onto Sonic
	gSonicState.task.pos.x = gMarioState->pos[0];
	gSonicState.task.pos.y = gMarioState->pos[1];
	gSonicState.task.pos.z = gMarioState->pos[2];
	
	//Update Sonic state
	for (i = 0; i < 2; i++)
		Sonic_Update();
	visRotate = Sonic_Animate();
	
	//Pre-update Mario code
	gMarioState->marioObj->header.gfx.node.flags &= ~GRAPH_RENDER_INVISIBLE;
	//mario_reset_bodystate(gMarioState);
	update_mario_inputs(gMarioState);
	mario_handle_special_floors(gMarioState);
	...
	//Apply Sonic state onto Mario
	gMarioState->pos[0] = gSonicState.task.pos.x;
	gMarioState->pos[1] = gSonicState.task.pos.y;
	gMarioState->pos[2] = gSonicState.task.pos.z;
	vec3f_copy(gMarioState->marioObj->header.gfx.pos, gMarioState->pos);
	
	gMarioState->forwardVel = gSonicState.spd.x * gSonicState.unitScale * i;
	gMarioState->vel[0] = gSonicState.gspd.x * gSonicState.unitScale * i;
	gMarioState->vel[1] = gSonicState.gspd.y * gSonicState.unitScale * i;
	gMarioState->vel[2] = gSonicState.gspd.z * gSonicState.unitScale * i;
	
    if (gSonicState.task.mode != SM_HURT && (gSonicState.task.flag & SF_DEAD) == 0)
        gMarioState->action = ACT_IDLE;
    else
        gMarioState->action = ACT_DISAPPEARED | ACT_FLAG_WATER_OR_TEXT;
	
	//Get camera facing direction
	look.x = 1.0f;
	look.y = 0.0f;
	look.z = 0.0f;
	Sonic_ConvertVector_P2G(&gSonicState, &look);
	
	if (look.x != 0.0f || look.z != 0.0f)
		gMarioState->faceAngle[1] = 0x4000 - atan2s(look.x, look.z);
	
	//Set Mario matrix
	njUnitMatrix(&(gMarioState->marioObj->transform[0][0]));
	Sonic_AngleToMatrix(&gMarioState->marioObj->transform[0][0], &gSonicState.task.ang);
	gMarioState->marioObj->transform[3][0] = gMarioState->pos[0];
	gMarioState->marioObj->transform[3][1] = gMarioState->pos[1];
	gMarioState->marioObj->transform[3][2] = gMarioState->pos[2];
	njRotateY(&(gMarioState->marioObj->transform[0][0]), 0x4000 + visRotate);
	gMarioState->marioObj->header.gfx.throwMatrix = gMarioState->marioObj->transform;
	
	//Post update Mario code
	sink_mario_in_quicksand(gMarioState);
	squish_mario_model(gMarioState);
	set_submerged_cam_preset_and_spawn_bubbles(gMarioState);
	update_mario_health(gMarioState);
	...
	if (gSonicState.floor != NULL)
	{
		//Use Sonic's current floor if standing on one
		gMarioState->floor = gSonicState.floor;
	}
	else
	{
		//Try to find a floor below our position
		gMarioState->floorHeight = find_floor(gMarioState->pos[0], gMarioState->pos[1], gMarioState->pos[2], &nextFloor);
		if (nextFloor != NULL)
			gMarioState->floor = nextFloor;
	}
...
void init_mario(void) {
    ...
    Sonic_Init(gMarioState);
    vec3f_copy(&gSonicState.task.pos.x, gMarioState->pos);
    Sonic_Turn(&gSonicState, 0x4000 - gMarioState->faceAngle[1]);
}
